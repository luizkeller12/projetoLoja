...
public class DaoLojaProduto {
    ...
    public void Salvar(LojaProdutoBean cid){
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("INSERT INTO prod_compra(quant_estoque,cod_loja,cod_produto)VALUES(?,?,?,?)");
    ...
    public void Editar(LojaProdutoBean cid ) {
        conex.conexao();    
        try {
            PreparedStatement pst = conex.con.prepareStatement("update prod_compra set quant_estoque=?,cod_loja=?, cod_produto=? where (cod_loja=? and cod_produto=?)");
           pst.setInt(1,cid.getQuant_estoque());
            pst.setInt(2, cid.getCod_loja());
            pst.setInt(3,cid.getCod_produto());
